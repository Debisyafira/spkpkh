        ...
        <div class="d-flex align-items-center">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb m-0 p-0">
              <li class="breadcrumb-item"><a href="{{ route('admin.ratioCriteria') }}" class="text-muted">Home</a></li>
            ...
            <div class="card">
              <div class="card-body">
                ...
                <div class="table-responsive mt-3">
                  <form class="form-group" method="POST" action="{{ route('admin.ratioCriteria.saveEigen') }}">
                    @csrf
                    <table class="table table-bordered table-responsive-lg">
                      <thead>
                        <tr>
                        ...
                        @foreach ($data->eigen as $keyName => $value)
                          <tr>
                            ...
                            @foreach ($value as $key => $valueMatrix)
                              @if ($key == 'totalEigen')
                                @if ($keyName != 'sumEigen')
                                  <input type="hidden" name="total_eigen['{{ $keyName }}']"
                                    value="{{ round($valueMatrix, 3) }}">
                                  <input type="hidden" name="average_eigen['{{ $keyName }}']"
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <button type="button" class="btn btn-dark" data-dismiss="modal" aria-label="Close"><i class="fa fa-times"></i>
          </button>
        </div>
